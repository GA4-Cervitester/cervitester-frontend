...
export default function ShowReport(props) {
    ...
    const fetchDoctors = (token) => {
        getDoctors(token)
        .then(response => {setDoctorsList(response)})
    ...
    const generatePDF = () => {
        ...
        if(data.modelFeedback === "Positive") result="Negative"
        if(data.modelFeedback === "Negative") result="Negative"
    ...
    return (
        <div>
            <div className='reportWrapper'>
                ...
                <div className='reportHeaderInfo'>
                    <Grid container>
                        <Grid className='reportHeaderItem' item xs={12} md={6}>
                            ...
                            <div>
                                 <strong>Last Update On:</strong> {formateTimeStamp(data.updatedTs)}
                            </div>
                        </Grid>
                        <Grid className='reportHeaderItem' item xs={12} md={6}>
                            <div>
                ...
                <hr/>
                <div className='cervicalCytologyReport'>
                    ...
                    <Grid container>
                        <Grid style={{marginBottom: "20px"}} item xs={12} md={6}>
                            <div style={{fontWeight: "600", fontSize: "1.rem"}}>
                                {data.testType==="0" ? (
                                    <span>Result : {data.modelFeedback}</span>
                                ): (
                                <span>
                                    Result: Negative
                                </span>)}
                        ...
                        <Grid item xs={12} md={6}>
                            <div>
                                Test Image:  &nbsp;
                                <img src={data.photoUri} alt="Test Input Loading.." className="reportImage"/>
                            </div>
                        </Grid>
                    </Grid>
                </div>
            ...
            <div className='showReportBottomButtons'>
                <Link href={`https://cervi-test-pccoe.herokuapp.com/detection/report?id=${data.id}`}>
                <Button
