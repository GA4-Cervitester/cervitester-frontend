...
export default function DoctorRequests(props) {
  ...
  const respondToRequest = (request) => {
    if(window.confirm("Do you want to accept the consultation request from patient #" + request.patient_id + " ?")){
        console.log("YES")
        requestRespond(accessToken, request.id, 1)
        .then(response=> {console.log(response)})
        .then(window.alert("Successfully accepted consultation request from patient #" + request.patient_id))
  ...
  const generateRows = (requests) => {
    ...
    requests.forEach((request)=>{
      ...
      let row = (
          <tr className='reportRow' key={request.id} onClick={()=>{respondToRequest(request)}}>
            <td>{request.id}</td>
