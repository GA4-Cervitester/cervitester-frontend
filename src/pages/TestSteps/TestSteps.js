...
import { FileUploadOutlined, LinkedCamera, ReportProblemOutlined } from '@mui/icons-material';
...
export default function TestSteps(props) {
    ...
    const loadModel = async () => {
        ...
        await tf.loadLayersModel("./verificationModel/model.json").then(res=>setModel(res));
    }
    ...
    useEffect(()=>{
        ...
        if(userType==="patient"){
            ...
            setSteps(["Select Patient", "Import Image", "Verify Image", "Upload Image"])
        }
    ...
    return (
        ...
        <div {...getRootProps({className: 'dropzone bigGridBox centerDiv1'})}>
            <input {...getInputProps()} id="imgUpload" onChange={(event)=> handleImgSelect(event.target.files[0])}/>
        ...
        return(
            <div className='testPatientPhoneContainer'>
                <TextField
                    ...
                    id="outlined-basic" label="Patient Phone Number" variant="outlined" fullWidth
                />
                <Button
                    className='linkPatientButton'
                    variant='contained'
                    onClick={()=>{getPatient(token, phone)}}
                >
                    Link Patient
                </Button>
                <div className='testPatientSelectMessage'>
                    {patientMessage}
                </div>
            </div>
        )
            ...
            <div className="testBoxesContainer">
                <Grid container spacing={2} className="centerDiv">
                    ...
                    {testType === 'Pap Smear' ? (
                        ...
                        <Grid item xs={xs} sm={sm} onClick={()=>{setWebcamModal(true)}}>
                    ...
                    {image !== null ? (
                        ...
                        <div className="pTestBoxText">
                                Your Image :
                        </div>
                            <img src={image} id="testImage" alt="" className="uploadedImage"/>
                        </div>
                        </Grid> 
                    ):null}
                    <WebcamCapture open={webcamModal} handleClose={closeWebcamModal} setWebcamImage={setImage}/>
                </Grid>
            </div>
        )
    }
    ...
    const verifyImageStep = () => {
        return (
            <div className="centerDiv" style={{flexDirection: 'column'}}>
                ...
                <img src={image} height="250px" alt="" style={{display: "flex"}} id="thisIMG"/>
                </div>
                {verified===true?(
                null
                ): (
                <div>
                <Button variant="contained" onClick={()=>{predictClass(model)}} style={{textTransform: "none", marginTop: "15px"}}>
                    Verify Image
                </Button>
                </div>
    ...
    const verifyImage = (prediction, testType) => {
        if (verified === true){
            if(prediction !== testType ){
                return (
                <div className="pTestCaution" style={{margin: "20px auto 40px auto"}}>
                    <div>
                    <ReportProblemOutlined fontSize="smaller" style={{marginRight: "5px"}}/>
                    Note: This seems to be a <strong>{prediction}</strong> test image, but you have selected <strong>{testType}</strong> test.
                    <br/> 
                    Please make sure you have uploaded the right image.
                    If you think this is a mistake, you can proceed further.
                    </div>
                </div>
                )
            }
    ...
    const lastStep = () => {
        ...
        return (
            <div className="centerText">
                ...
                <span>You will find your report in the <b>Reports</b> tab shortly.</span>
        ...
        <div>
            ...
            {steps.map((label, index) => {
                ...
                <Step key={label} {...stepProps}>
                    <StepLabel className="testStepperLabel" {...labelProps}>{label}</StepLabel>
                </Step>
                );
            })}
            </Stepper>
            <div className="testStepsContent">
                {userType==="patient" ? (
                    displayStepsforPatient(activeStep)
                ):null}
                {userType==="doctor" ? (
                    displayStepsforDoctor(activeStep)
                ):null}
            </div>
