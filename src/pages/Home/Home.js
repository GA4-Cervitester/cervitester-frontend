...
import { fetchReports, fetchProfile, fetchRequests } from '../../store/reducers/userData'
...
export default function Home() {
    ...
    const requests = requestsUnfiltered.filter(requestsUnfiltered=> requestsUnfiltered.status !== "yes")
    ...
    const reportsTable = () => {
        if(reports.length === 0){
            return (
                <div> You have no reports at the moment... </div>
            )
        }
        return (
            <div>
                ...
                <div className='centerText' style={{marginTop: "25px", fontSize: "1.2rem", color: "#3E95E1"}}>
    ...
    const requestsTable = () => {
        if(requests.length === 0){
            return (
                <div> You have no requests at the moment... </div>
            )
        }
        return (
            <div>
                ...
                <Table hover>
                    ...
                    <tbody>
                        {generateRequestRows(requests)}
                    </tbody>
                </Table>
                <div className='centerText' style={{marginTop: "25px", fontSize: "1.2rem", color: "#3E95E1"}}>
                    <NavLink to={{pathname: '/doctor-requests'}}>
                        See all &gt;
                    </NavLink>
                </div>
            </div>
    ...
    const homeProfileCard = () => {
        return (
            <div className='centerDiv1'>
                <img className='homeProfileImg' src='./images/woman.png' alt='profile'/>
